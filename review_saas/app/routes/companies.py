...
async def add_company(
    ...
    with SessionLocal() as s:
        if place_id:
            dup = (
                s.query(Company)
                .filter(and_(Company.owner_id == user_id, Company.place_id == place_id))
                .first()
            )
            if dup:
                raise HTTPException(status_code=400, detail="Company exists (place_id)")
        if name:
            dup2 = (
                s.query(Company)
                .filter(and_(Company.owner_id == user_id, Company.name == name, Company.city == city))
                .first()
            )
            if dup2:
                raise HTTPException(status_code=400, detail="Company exists (name/city)")
        c = Company(
            owner_id=user_id,
...
def list_companies(q: str | None = None, status: str | None = None, user_id: int = Depends(get_current_user_id)):
    with SessionLocal() as s:
        ...
        if q:
            like = f"%{q}%"
            query = query.filter(
                (Company.name.like(like)) | (Company.city.like(like)) | (Company.place_id.like(like))
            )
        if status:
