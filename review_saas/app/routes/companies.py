...
async def companies_page(request: Request, q: str | None = None, page: int = 1, size: int = 10):
    ...
    return templates.TemplateResponse('companies.html', {
        ...
        "size": size, "total": total, "q": q or ''
    })

# --- MANUAL SYNC (Uses Background Task) ---
...
async def company_sync(company_id: int, request: Request, bg_tasks: BackgroundTasks):
    ...
    async with get_session() as session:
        ...
        # Trigger background fetch using the IDs
        bg_tasks.add_task(ingest_company_reviews, session, c.id, c.place_id)
        
        session.add(AuditLog(user_id=uid, action='company_sync_triggered', meta={'company_id': c.id}))
...
# --- ADD FROM GOOGLE SEARCH (World-Class Integration) ---
...
async def add_new_company(request: Request, data: AddCompanyRequest, bg_tasks: BackgroundTasks):
    uid = _require_user(request)
    if not uid: return {"success": False, "message": "Unauthorized"}
    ...
    async with get_session() as session:
        ...
        return {
            ...
            "message": "Company added and reviews are loading!"
        }
