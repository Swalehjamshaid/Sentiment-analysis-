from fastapi import APIRouter, HTTPException, Depends
    ...
    @router.post("")
    def add_company(name: str | None = None, maps_link: str | None = None, place_id: str | None = None, city: str | None = None, user_id: int = Depends(get_current_user_id)):
        ...
        with SessionLocal() as s:
            if place_id:
                ...
                if dup: raise HTTPException(status_code=400, detail="Company already exists for this user (place_id)")
            if name:
                dup2 = s.query(Company).filter(and_(Company.owner_id==user_id, Company.name==name, Company.city==city)).first()
                if dup2: raise HTTPException(status_code=400, detail="Company already exists for this user (name/city)")
    ...
    def update_company(company_id: int, name: str | None = None, city: str | None = None, status: str | None = None, user_id: int = Depends(get_current_user_id)):
        with SessionLocal() as s:
            ...
            if not c or c.owner_id != user_id: raise HTTPException(status_code=404, detail="Not found")
