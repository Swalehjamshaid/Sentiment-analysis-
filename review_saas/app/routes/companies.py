...
async def google_autocomplete(request: Request, input: str = Query(...)):
    ...
    if not client:
        return JSONResponse(status_code=503, content={"error": "Google Maps client not configured"})
    try:
        ...
        return JSONResponse(status_code=500, content={"error": "Autocomplete service unavailable"})

# ─────────────────────────────────────────
# Google Place Details
# ─────────────────────────────────────────
@router.get("/google/place/details")
async def google_place_details_endpoint(request: Request, place_id: str = Query(...)):
    _require_user(request)
...
async def add_company(request: Request, background: BackgroundTasks, company_in: CompanyCreate):
    ...
    # Background review ingestion using Google API
