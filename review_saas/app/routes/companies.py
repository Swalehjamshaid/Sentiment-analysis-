...
async def edit_company(
    ...
    if payload.name is not None: c.name = bleach.clean(payload.name, strip=True)
    if payload.city is not None: c.city = bleach.clean(payload.city, strip=True)
...
async def upload_logo(
    ...
    safe_name = f"{int(datetime.utcnow().timestamp())}_{os.path.basename(logo.filename)}"
