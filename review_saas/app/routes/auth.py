...
# OAuth is optional; guard import so missing config won't break startup
try:
    from ..utils.oauth import oauth  # authlib OAuth client (registered only if env vars exist)
...
async def register(
    ...
    """Create user, send (and also return) a verify token for dev."""
    ...
    with SessionLocal() as s:
        ...
        # Return token in response as a convenience for dev environments
...
def verify(token: str):
    with SessionLocal() as s:
        vt = (
            s.query(VerificationToken)
            .filter(VerificationToken.token == token, VerificationToken.used == False)  # noqa: E712
            .first()
        )
...
def login(request: Request, email: str, password: str, totp: str | None = None):
    """Password login + optional TOTP when ENABLE_2FA=1 and user.twofa_secret is set."""
...
def reset(token: str, new_password: str, confirm_password: str):
    ...
    with SessionLocal() as s:
        rt = (
            s.query(ResetToken)
            .filter(ResetToken.token == token, ResetToken.used == False)  # noqa: E712
            .first()
        )
...
# ---------- Optional Google OAuth (guarded) ----------
@router.get("/google/login")
async def google_login(request: Request):
    # Only available if OAuth is configured and Google is registered
