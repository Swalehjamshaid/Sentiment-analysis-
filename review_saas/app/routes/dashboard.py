...
async function loadCompanies() {
  try {
    ...
    if (!data.companies || data.companies.length === 0) {
      el.companySelect.innerHTML = '<option value="">No companies found</option>';
      return;
    }
    ...
    el.companySelect.value = savedId && data.companies.some(c=>c.id==savedId) ? savedId : data.companies[0].id;
...
async function fetchDashboardData() {
  ...
  try {
    ...
    const latestValue = (revSeries.series && revSeries.series.length>0) ? revSeries.series.at(-1).value : 0;
    el.kpiNew.textContent = latestValue;

    Object.values(charts).forEach(c => c?.destroy?.());

    const allDates = generateDateRange(el.startDate.value, el.endDate.value);
    const revMap = new Map(revSeries.series.map(x=>[x.date,x.value]));
    const revData = allDates.map(d => revMap.get(d) ?? 0);

    const sentMap = new Map(sentSeries.series.map(x=>[x.date,x.value]));
    const sentData = allDates.map(d => sentMap.get(d) ?? 0);

    charts.rev = new Chart(document.getElementById('seriesReviews'), {
      type:'line',
      data:{labels:allDates,datasets:[{label:'Reviews',data:revData,borderColor:'#0d6efd',backgroundColor:'rgba(13,110,253,0.1)',tension:0.3,fill:true}]},
    ...
    charts.sent = new Chart(document.getElementById('sentSeries'), {
      type:'line',
      data:{labels:allDates,datasets:[{label:'Sentiment',data:sentData,borderColor:'#6610f2',backgroundColor:'rgba(102,16,242,0.1)',tension:0.3,fill:true}]},
