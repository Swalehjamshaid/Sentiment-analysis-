...
def _quick_range(range_key: Optional[str]):
    if not range_key: return None, None
...
async def dashboard_page(
    ...
    # Cast to int for PG compatibility
    try:
    ...
    params = {"from": from_ or (sdt.date().isoformat() if sdt else ""), 
              "to": to or (edt.date().isoformat() if edt else ""), "range": range or ""}
    ...
    reply_rows = db.query(models.Reply).join(models.Review).filter(models.Review.company_id == company_id).all()
    for rp in reply_rows:
        if rp.review_id not in reply_map or rp.replied_at > reply_map[rp.review_id].replied_at:
    ...
    for r in reviews:
        ...
        review_vm.append(SimpleNamespace(
            id=r.id, review_date=r.review_date, reviewer_name=r.reviewer_name, rating=r.rating,
            sentiment_category=r.sentiment_category, sentiment_score=r.sentiment_score,
            sentiment_confidence=r.sentiment_confidence, emotion_label=r.emotion_label,
            is_spam_suspected=r.is_spam_suspected, aspect_summary=r.aspect_summary,
            topics=r.topics, keywords=r.keywords, language=r.language, text=r.text,
    ...
    health_data = db.query(models.ApiHealthCheck).filter(models.ApiHealthCheck.company_id == company_id).order_by(models.ApiHealthCheck.last_checked_at.desc()).all()
    api_health = [{"provider": h.provider, "status": h.status} for h in health_data]
    alert_rows = db.query(models.Alert).filter(models.Alert.company_id == company_id).order_by(models.Alert.triggered_at.desc()).limit(10).all()

    try:
        from app.services.rbac import get_user_roles_for_company
        roles_list = get_user_roles_for_company(db, current_user, company_id)
    except Exception: roles_list = []

    final_ctx = common_context(request)
    final_ctx.update({
        "companies": companies, "active_company": active, "params": params, "kpi": kpi,
        "charts": charts, "reviews": review_vm, "summary": ai_summary.get("summary_text"),
        "api_health": api_health, "roles": roles_list, "alerts": alert_rows,
    })
