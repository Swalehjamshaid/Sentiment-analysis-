...
async def api_series_reviews(company_id: int, start: str | None = None, end: str | None = None):
    ...
    async with get_session() as session:
        ...
        stmt = select(date_col.label("date"), func.count(Review.id).label("value")) \
               .where(Review.company_id == company_id)
...
async def api_sentiment_series(company_id: int, start: str | None = None, end: str | None = None):
    ...
    async with get_session() as session:
        date_col = cast(Review.google_review_time, Date)
        stmt = select(date_col.label("date"), func.avg(Review.sentiment_score).label("value")) \
               .where(Review.company_id == company_id)
        if start_dt: stmt = stmt.where(date_col >= start_dt)
        if end_dt:   stmt = stmt.where(date_col <= end_dt)
        stmt = stmt.group_by("date").order_by("date")
...
# ────────────────────────────────────────────────
#  NEW – Decision making endpoints
# ────────────────────────────────────────────────

@router.get("/api/aspects/avg")
async def api_aspects_average(
    company_id: int,
    start: str | None = None,
    end: str | None = None
):
    """Average sentiment per aspect – very important for owners"""
    start_dt, end_dt = parse_date(start), parse_date(end)
...
async def api_complaints_stats(
    company_id: int,
    start: str | None = None,
    end: str | None = None
):
    ...
    async with get_session() as session:
        ...
        complaints = await session.execute(
            select(func.count()).select_from(base.subquery()).where(Review.is_complaint == True)
        )
        ...
        return {
            ...
            "praise_count": await session.execute(
                select(func.count()).select_from(base.subquery()).where(Review.is_praise == True)
            ).scalar() or 0
        }


@router.get("/api/keywords/top")
async def api_top_keywords(
    company_id: int,
    start: str | None = None,
    end: str | None = None,
    limit: int = Query(12, ge=5, le=30)
):
    """Very simple top words – better with proper NLP preprocessing in future"""
    start_dt, end_dt = parse_date(start), parse_date(end)
    ...
    async with get_session() as session:
        ...
        # Very naive top words (improve later with proper tokenization + stop words)
        ...
        # Very crude classification – improve with sentiment lexicon or model
...
async def api_simple_recommendations(company_id: int, start: str | None = None, end: str | None = None):
    """Rule-based basic recommendations – extend later with more logic"""
    ...
    weak_aspects = []
    for aspect, score in aspects["aspects"].items():
        if score < 0.1 and score is not None:
    ...
    if kpis["avg_sentiment"] < 0.05 and kpis["avg_rating"] >= 4.0:
        recs.append("High rating but low/near-zero sentiment → guests are satisfied but not delighted → look for ways to create wow moments")
