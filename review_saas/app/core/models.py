...
class Company(Base):
    ...
    # Details from Google Places API
    ...
    # Aggregated Analytics (Stored/Cached)
    avg_rating: Mapped[float | None] = mapped_column(Float, nullable=True)
    review_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
    
    # IMPORTANT: Added these fields to match your SQL query logs and prevent 500 errors
    avg_sentiment: Mapped[float | None] = mapped_column(Float, nullable=True) 
    last_review_fetched_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class Notification(Base):
    ...
    # Requirement: info, alert (significant rating drop)
...
class AuditLog(Base):
    ...
    # Requirement: Maintain audit logs (GDPR Compliance)
