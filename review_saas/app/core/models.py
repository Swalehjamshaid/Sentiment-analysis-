...
class Company(Base):
    ...
    # Unique Google Place ID for API synchronization
    place_id: Mapped[str] = mapped_column(String(255), unique=True, index=True)
    
    # Business details fetched from Google
    ...
    # Existing Analytic Columns (Synchronized with DB)
...
class Review(Base):
    ...
    # Prevent duplicate review entries for the same company
    __table_args__ = (UniqueConstraint("company_id", "source_id", name="uq_review_source"),)
    
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    company_id: Mapped[int] = mapped_column(ForeignKey("companies.id", ondelete="CASCADE"), index=True)
    source_id: Mapped[str] = mapped_column(String(255), index=True) # ID from Google
    ...
    # Sentiment Analysis data used for Dashboard Cards
...
class Notification(Base):
    ...
    type: Mapped[str] = mapped_column(String(64)) # info | alert
...
class AuditLog(Base):
    ...
    action: Mapped[str] = mapped_column(String(128)) # e.g., "ADD_COMPANY", "LOGIN"
