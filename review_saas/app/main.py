...
# ---------------------------------------------------
# 1. PATH RESOLUTION (SAFE VERSION - NO OVERLOAD)
# ---------------------------------------------------
...
# Only add if missing (prevents import conflicts)
...
try:
    ...
    logger.critical("❌ CRITICAL IMPORT FAILURE")
    traceback.print_exc()
    raise
...
# ---------------------------------------------------
# 5. LIFESPAN (HARDENED - NO SILENT FAILURES)
# ---------------------------------------------------
@asynccontextmanager
    ...
    try:
        ...
        async with SessionLocal() as session:
            try:
                ...
                if old_v != new_v:
                    logger.warning(f"🧩 Schema Mismatch: {old_v} → {new_v}")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "app.main:app",   # ✅ FIXED (was breaking earlier)
        host="0.0.0.0",
