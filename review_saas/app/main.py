...
# Import db instance (but do NOT init_app yet)
from . import db  # assuming db = SQLAlchemy() is in __init__.py or separate file
...
def wait_for_db():
    """Wait for PostgreSQL to be ready before initializing DB."""
...
def create_app() -> Flask:
    ...
    # Wait for DB before initializing (critical for Railway)
    wait_for_db()
    db.init_app(app)
    ...
    app.register_blueprint(health_bp)  # healthcheck - must be independent
