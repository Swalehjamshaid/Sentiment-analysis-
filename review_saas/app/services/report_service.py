# filename: app/services/report_service.py
...
def _generate_charts(
    ...
    bar_path = os.path.join(
        self.output_dir,
        f"{company_name}_rating_bar.png"
    )
...
def _generate_summary(
    ...
    summary = f"""
    {company_name} currently maintains an average customer
    rating of {analytics['average_rating']} based on
    {analytics['total_reviews']} customer reviews.

    Positive customer sentiment stands at
    {analytics['positive_percent']}%, while
    negative sentiment accounts for
    {analytics['negative_percent']}%.

    The business demonstrates strong customer engagement,
    however operational improvements are recommended to
    reduce negative feedback and strengthen customer
    retention.
    """
...
def _generate_recommendations(
    ...
    if analytics["negative_percent"] > 25:
        ...
        recommendations.append(
            "Increase customer support response speed."
        )

        recommendations.append(
            "Conduct staff customer handling training."
        )
    ...
    if analytics["average_rating"] < 4:
        ...
        recommendations.append(
            "Launch customer satisfaction campaigns."
        )
    ...
    if not recommendations:
        ...
        recommendations.append(
            "Maintain current service quality and monitor trends closely."
        )
...
def _generate_action_plan(
    ...
    return [
        {
            "week": "Week 1",
            "task": "Review negative feedback and identify recurring operational issues."
        },
        {
            "week": "Week 2",
            "task": "Conduct customer service training for staff."
        },
        {
            "week": "Week 3",
            "task": "Improve operational efficiency and customer response time."
        },
        {
            "week": "Week 4",
            "task": "Launch customer engagement and retention campaigns."
        },
    ]
...
def _build_pdf(
    ...
    story.append(
        Paragraph(
            f"Executive Business Intelligence Report - {company_name}",
            title_style
        )
    )
    ...
    kpi_data = [
        ...
        ["Neutral Reviews", analytics['neutral_reviews']],
        ["Negative Reviews", analytics['negative_reviews']],
        ["Positive %", f"{analytics['positive_percent']}%"],
        ["Negative %", f"{analytics['negative_percent']}%"],
    ]
    ...
    if os.path.exists(chart_paths['pie_chart']):
    ...
    if os.path.exists(chart_paths['bar_chart']):
    ...
    story.append(
        Paragraph(
            "Generated by TrustlyticsAI Executive Intelligence Engine",
            body_style
        )
    )
