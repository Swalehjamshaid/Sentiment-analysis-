...
async def fetch_reviews(place_id: str, limit: int = 1000) -> List[Dict[str, Any]]:
    """
    REGIONAL STEALTH V3: 
    Aligned function name to fix ImportError in app/routes/reviews.py.
    ...
    async with httpx.AsyncClient(headers=headers, timeout=30.0, follow_redirects=True) as client:
        while len(all_reviews) < limit:
            # Secure HTTPS path for Google Pakistan
            ...
            try:
                ...
                if response.status_code != 200:
                    logger.error(f"🕵️ Regional Wall hit (Status {response.status_code}) at offset {offset}")
                    break
                ...
                offset += 100
                # Human-like delay to avoid being flagged
