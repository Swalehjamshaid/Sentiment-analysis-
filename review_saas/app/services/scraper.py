...
async def fetch_reviews(place_id: str, limit: int = 50):
    ...
    Aligned with Review.py model mapping.
    """
    ...
    async with async_playwright() as p:
        # Launching Chromium with required Cloud/Railway flags
        ...
        # --- NETWORK DATA INTERCEPTION (The 'Logic' from the Video) ---
        async def handle_response(response):
            if "batchexecute" in response.url:
                try:
                    text = await response.text()
                    # Strip Google's anti-hijack prefix
                    cleaned_text = text.replace(")]}'", "").strip()
                    
                    # Intercepting the 'wrb.fr' data arrays containing review objects
                    matches = re.findall(r'\["wrb\.fr".*?\]\]', cleaned_text)
                    
                    for match in matches:
                        try:
                            # Double-decode the nested JSON structure
                                    ...
                                    for r in block:
                                        try:
                                            ...
                                            if r_id not in visited_ids:
                                                # DATA MAPPING: Synchronized with Backend routes
                                                reviews_data.append({
                                                    "review_id": r_id,           
                                                    "author_name": r[1][0],      
                                                    "rating": r[4],             
        ...
        try:
            ...
            while len(reviews_data) < limit and scrolls < max_scrolls:
                # Scroll to trigger lazy loading of batchexecute requests
