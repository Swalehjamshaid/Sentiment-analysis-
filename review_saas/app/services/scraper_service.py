...
class FastGoogleScraper:
    ...
    async def get_reviews(self, data_id: str, limit: int = 10000) -> List[Dict[str, Any]]:
        """
        Fetches up to 10,000 reviews using Google's internal AJAX endpoint.
        """
        ...
        async with httpx.AsyncClient(headers=self.headers, timeout=60.0, follow_redirects=True) as client:
            try:
                ...
                if response.status_code != 200:
                    logger.error(f"Scraper Error: Status {response.status_code}")
                    return []
                ...
                # Heuristic search for the review block
                for item in data:
                ...
                for r in raw_reviews:
                    try:
                        author_title = r[1][4][0][4] if (len(r) > 1 and r[1] and len(r[1]) > 4) else "Google User"
                        reviews_list.append({
                            "review_id": str(r[0]),
        ...
        logger.info(f"✅ Scraper fetched {len(reviews_list)} reviews for {data_id}")
        return reviews_list
