...
class FastGoogleScraper:
    ...
    async def get_reviews(self, data_id: str, limit: int = 1000) -> List[Dict[str, Any]]:
        """
        ...
        :param limit: Number of reviews to fetch. Increased default to 1000.
        """
        ...
        # Increase timeout to 30.0 to handle larger data sets
        async with httpx.AsyncClient(headers=self.headers, timeout=30.0, follow_redirects=True) as client:
            try:
                ...
                # Google Maps AJAX responses are deeply nested lists
                ...
                # It usually resides in a sub-list that contains author information
                raw_reviews = []
                for item in data:
                ...
                for r in raw_reviews:
                    try:
                        # Extract the unique review ID
                        review_id = str(r[0])
                        
                        # Author extraction
                        # Structure: [ [ [author_data] ] ]
                        author_title = "Google User"
                        if len(r) > 1 and r[1] and len(r[1]) > 4:
                            author_title = r[1][4][0][4]

                        # Review Item mapping to match Database model
                        review_item = {
                            "review_id": review_id,
                        ...
                        # Skip malformed individual review entries without crashing the whole process
                        continue
        ...
        logger.info(f"📊 Scraper retrieved {len(reviews_list)} reviews for ID: {data_id}")
        return reviews_list
