...
def ingest_company_reviews(
    place_id: str,           # ← FIXED: added place_id as first required parameter
    company_id: str,
    api_client,
    start_date: Optional[datetime] = None,
    end_date: Optional[datetime] = None,
    ...
    reviews_data = service.fetch_reviews(
        place_id=place_id,           # ← now passed correctly
    ...
    logger.info(f"Fetched {len(company_reviews.reviews)} reviews for company {company_id} (place_id: {place_id})")
